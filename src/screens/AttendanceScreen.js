  ...
  const handleCheckInOut = async () => {
    ...
    try {
      ...
      if (
        ...
        setCheckOut(convertUTCtoIST(response.result.attendance.check_out));
        ...
        setModalVisible(false);     {/* set module (or) CheckInOut pop-up window false  */}
        
                      setLoading(false);             {/* set loding off  when the operation done successfully */}
                      setIsButtonDisabled(false);
                    } catch (error) {                     
                  ...
                  error => {                               {/* show error when GeoLocation not found */}
                    setLoading(false);
        ...
        // this is when the application state   is unequal      with surver state.
        ...
        if (employeeAttendanceState === 'checked_in') {
          Alert.alert(`You are alrady "Check In".`);
        } else {
          Alert.alert(`You are alrady "Check Out".`);
        }
  ...
  return (
    <View style={{alignItems: 'center', marginTop: 20}}>
            ...
            <Text style={{marginTop: 10, fontSize: 16}}>
              Worked Hours: { workedHours }                    {/* you can also use here   "checkInDuration"  if you want it in format "hh.mm.ss" */}
            </Text>
